<html><head><title>图书借出</title>
<meta http-equiv="content-type" content="text/html"; charset="utf-8">
</head><body>
	<a href = 'main.php'>返回主菜单</a> &nbsp;
	<h2 align='center' >图书借出</h2>
...
<?php
	...
	if(isset($_POST['Submit'])){
		...
		$stmt2 = "SELECT * FROM book WHERE bno = '$bno'";
		...
		if($row = sqlsrv_fetch_array($result)){
			if($row2 = sqlsrv_fetch_array($result2)){
				...
				$_SESSION['cno'] = $cno;
				$_SESSION['bno'] = $bno;
				header("location: http://localhost:8080/lib%20manage/book_out%202.php");
			}
			else{
				echo "<script>alert('书目不存在！');</script>";
			}
		}
		else{
